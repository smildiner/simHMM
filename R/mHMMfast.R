...
mHMMfast <- function(s_data, gen, xx = NULL, start_val, mcmc, return_path = FALSE, print_iter, show_progress = TRUE,
    ...
    for (iter in 2 : J){
        ...
        for(i in 1:m){
            ...
            for (s in 1:n_subj){
                if (is.null(trans[[s]][[i]]) | (length(trans[[s]][[i]]) == 0)) {
                ...
                gamma[[s]][i,]  	<- PD_subj[[s]][iter, c((sum(m * q_emiss) + 1 + (i - 1) * m):(sum(m * q_emiss) + (i - 1) * m + m))] <- gamma_RWout$prob + 0.0001
                ...
                start <- c(0, q_emiss * m)
                for(q in 1:n_dep){
                    if (is.null(cond_y[[s]][[i]][[q]]) | (length(cond_y[[s]][[i]][[q]]) == 0)) {
                    ...
                    emiss[[s]][[q]][i,]		   <- PD_subj[[s]][iter, (sum(start[1:q]) + 1 + (i - 1) * q_emiss[q]):(sum(start[1:q]) + (i - 1) * q_emiss[q] + q_emiss[q])] <- emiss_RWout$prob + 0.0001
